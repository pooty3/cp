...
bool isConvex(const vector<point> &P) {
  ...
  if (n <= 3) return false;
  bool firstTurn, found = false;
  //find first non collinear point
  for (int i = 0; i < n-1;++i) {
  	if (!collinear(P[i], P[i+1], P[(i+2) == n ? 1 : i+2])) {
  		firstTurn = ccw(P[i], P[i+1], P[(i+2) == n ? 1 : i+2]);
  		found = true; break;
  	}
  }
  if (!found) return false;      // remember one result,
  for (int i = 1; i < n-1; ++i)   
  	//do collinear check
  	if (collinear(P[i], P[i+1], P[(i+2) == n ? 1 : i+2])) continue;             // compare with the others
